...
def stochastic_snapshot(snapshot, starting_round, n_round, table, options):
    ...
    for round in range(starting_round, n_round + 1):
        ...
        start_time_log_file_suffix = options.remote_probe_hostname + "_start_time_log_file_" + str(snapshot) + "_" + str(round) + ".log"
        pcap_file_suffix = options.remote_probe_hostname + "_" + options.pcap_file_prefix + str(snapshot) + "_" + str(round) + ".pcap"
        shuffled_csv_file_suffix = options.remote_probe_hostname + "_" + options.probes_file_prefix + str(snapshot) + "_" + str(round) + "_shuffled.csv"
        shuffled_next_round_csv_file_suffix = options.remote_probe_hostname + "_" + options.probes_file_prefix + str(snapshot) + "_" + str(round + 1) + "_shuffled.csv"


        # Those files are local to the central server
        replies_csv_file = resources_dir + options.remote_probe_hostname + "_" + options.replies_file_prefix + str(snapshot) + "_" + str(
            round) + ".csv"
        next_round_csv_file = resources_dir + options.remote_probe_hostname + "_" + options.probes_file_prefix + str(snapshot) + "_" + str(
            round + 1) + ".csv"
        ...
        if not options.only_analyse:
            probe(pcap_file, shuffled_probes_csv_file, start_time_log_file, options, is_stochastic)
            if options.is_remote_probe and round > 1:
                remote_shuffled_probes_csv_file = remote_resources_dir + shuffled_csv_file_suffix
            ...
            if options.is_remote_probe:
                ...
                print("scp " + remote_pcap_file + " time: " + str(end_scp_time - scp_time) + " seconds.")
                ...
                if options.proto == "tcp":
                    scp_time = time.time()
                    remote_start_time_log_file = remote_resources_dir + start_time_log_file_suffix
                    local_start_time_log_file = resources_dir + start_time_log_file_suffix
                    prober_to_server(remote_start_time_log_file, local_start_time_log_file, options)
                    end_scp_time = time.time()
                    print("scp " + remote_start_time_log_file + " time: " + str(end_scp_time - scp_time) + " seconds.")
        ...
        local_start_time_log_file = resources_dir + start_time_log_file_suffix
        pcap_to_csv(snapshot, round, local_pcap_file, replies_csv_file, local_start_time_log_file, options)
        end_pcap_to_csv_time = time.time()
        print("pcap_to_csv round " + str(round) + " time: " + str(end_pcap_to_csv_time - start_pcap_to_csv_time) + " seconds.")
        ...
        print("next_round_csv round " + str(round) + " time: " + str(end_next_round_csv_time - start_next_round_csv_time) + " seconds.")
        ...
        local_shuffled_next_round_csv_file = resources_dir + shuffled_next_round_csv_file_suffix
        shuffle_next_round_csv(next_round_csv_file, local_shuffled_next_round_csv_file, options)
        ...
        if options.is_remote_probe and not options.only_analyse:
            ...
            scp_time = time.time()
            local_shuffled_next_round_csv_file = resources_dir + shuffled_next_round_csv_file_suffix
            remote_shuffled_next_round_csv_file = remote_resources_dir + shuffled_next_round_csv_file_suffix
            next_round_server_to_prober_csv(local_shuffled_next_round_csv_file, remote_shuffled_next_round_csv_file, options)
            end_scp_time = time.time()
            print("scp pcap round " + str(round) + " time: " + str(end_scp_time - scp_time) + " seconds.")
...
if __name__ == "__main__":
    ...
    for snapshot in range(options.stochastic_snapshot_number, n_snapshots + 1):
        ...
        for i in range(len(snapshot_nodes)):
            ...
            node, user, home_dir, remote_resources_dir = snapshot_nodes[i]
            ...
            if node == localhost:
                ...
                options_node.heartbeat_binary = options.heartbeat_dir + "build/Heartbeat"
                ...
                options_node.heartbeat_binary = options_node.probe_dir + "build/Heartbeat"
                options_node.db_table += "_" + str(int(time.time()))
