...
def shuffle_next_round_csv(csv_file, shuffled_csv_file, options):
    shuffle_next_round_csv_cmd = (
        "export MEMORY=40; export TMPDIR="
        + options.home_dir
        + "Heartbeat/resources/; "
        + options.home_dir
        + "terashuf/terashuf < "
    ...
    shuffle_next_round_csv_process = Popen(
        shuffle_next_round_csv_cmd, stdout=PIPE, stderr=PIPE, shell=True
    )
...
def next_round_csv(snapshot, round, csv_file, table, options):
    next_round_csv_cmd = (
        ...
        + options.db_host
        + " -v "
        + str(int(ipaddress.IPv4Address(options.remote_probe_ip)))
        + " --dport="
        + str(options.dport)
        + " --skip-prefixes="
    ...
    next_round_csv_process = Popen(
        next_round_csv_cmd, stdout=PIPE, stderr=PIPE, shell=True
    )
...
def insert_csv_to_db(csv_file, table, options):
    ...
    insert_csv_to_db_process = Popen(
        insert_csv_to_db_cmd, stdout=PIPE, stderr=PIPE, shell=True
    )
...
def pcap_to_csv(snapshot, round, pcap_file, csv_file, start_time_log_file, options):
    ...
    pcap_to_csv_process = Popen(pcap_to_csv_cmd, stdout=PIPE, stderr=PIPE, shell=True)
...
def probe(pcap_file, csv_file, start_time_log_file, options, is_stochastic):
    ...
    if options.is_remote_probe:
        ...
        client.connect(
            options.remote_probe_hostname, username=options.remote_probe_user
        )
        # get a session
        ...
        stdin, stdout, stderr = client.exec_command(
            "echo " + options.remote_probe_host_sudo_password + " | " + probe_cmd
        )
        ...
        if options.targets is not None:
            probe_cmd += " -T -t " + options.targets
        print("Executing " + probe_cmd)
        probe_process = Popen(probe_cmd, stdout=PIPE, stderr=PIPE, shell=True)
